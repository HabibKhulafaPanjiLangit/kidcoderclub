...
const UsersPage: React.FC = () => {
  ...
  // Fungsi approve dan reject user
  const handleApprove = async (userId: string) => {
    if (!window.confirm('Approve user ini?')) return;
    setLoading(true);
    const { error } = await supabase
      .from('users')
      .update({ status: 'approved' })
      .eq('id', userId);
    if (error) alert('Gagal approve user: ' + error.message);
    else alert('User berhasil di-approve!');
    // Refresh data
  ...
  const handleReject = async (userId: string) => {
    if (!window.confirm('Reject user ini?')) return;
    setLoading(true);
    const { error } = await supabase
      .from('users')
      .update({ status: 'rejected' })
      .eq('id', userId);
    if (error) alert('Gagal reject user: ' + error.message);
    else alert('User berhasil di-reject!');
    // Refresh data
    const { data } = await supabase
      .from('users')
      .select('id, name, email, role, status, phone, created_at');
